...
```{r load_packages, message=F, include=T}
...
```{r}
# Read in GTF
gtf <- readGFF(filepath=paste0(params$data,"/Homo_sapiens.GRCh37.74.gtf.gz")) 
...
counts <- data.table(
  read.csv(file = paste0(params$data,
...
```{r}
bat = data.table(
  read.csv(file = paste0(params$data,"/MMRF_CoMMpass_IA14_Seq_QC_Summary.csv"))
...
```{r}
pat = data.table(
  read.csv(file = paste0(params$data,"/MMRF_CoMMpass_IA14_PER_PATIENT.csv")
...
dq.genes = read.csv(file = paste0(params$data,"/Union_Discordant_genes_TCGA_GTEx_Arora_TS2B.csv")) %>% data.table()
...
pca.score = cbind(cbat.dt$SEQ_ID,pca$x[,elbow[selected=='Selected']$pc]) %>% data.table() # PC value for each baseline sample
colnames(pca.score)[1] = "SEQ_ID" # Annotate sample names
write.csv(pca.score,file = paste0(params$data,
...
write.csv(pca.genes,file = paste0(params$data,
...
write.csv(exp_cbat,file = paste0(params$data,
...
write.csv(exp_time,file = paste0(params$data,
