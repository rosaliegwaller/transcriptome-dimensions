...
setwd("C:/Users/rosal/OneDrive - University of Utah/2020/disseminate/manuscripts/first-author/myeloma-tumor-rna-dimensions/supplement/Rmd_Notebooks_2020-08-20")
```
...
Plot fitted values x actual values
```{r}
p1 = ggplot(pdt,aes(y = combined, x = score, shape = smp, color = smp)) +
  geom_point() +
  scale_color_manual(values = c('#FF000060','#FF0000','#FF0000')) +
  theme_classic()+
  xlab("SBUK-17 Score") + ylab("Predicted Score") +
  theme(legend.position="none",
        axis.title=element_text(size=8))
p1
ggsave(filename = "plots/SBUK-17_dot.pdf",plot = p1,width=2.5,height=2.5)
```
...
# List dimensions significant in both
sig.both = intersect(sig.17,sig.70)
```

Plot dimension profile for extreme samples
```{r}
# use same extreme samples
dt.melt = melt(dt,id.vars = c("SEQ_ID","score","combined"))
dt.melt[,sig:=as.factor('N')]
dt.melt[variable%in%sig.17]$sig = 'Y'
dt.melt$variable = as.factor(dt.melt$variable)

p1 = ggplot(data=dt.melt[dt.melt$variable%like%"_SD" & dt.melt$SEQ_ID=="MMRF_2106_1_BM",], #MMRF_2722_1_BM 
             aes(x=variable, y=value, fill=sig)) + 
  geom_bar(stat="identity") +
  ylim(-3,3) +
  xlab("Dimension") +
  ...
  theme(axis.text=element_blank(),
        ...
        axis.title=element_text(size=8))
p1
ggsave(filename = "plots/SBUK-17_MMRF-2106.png",plot = p1,width=2.5,height=1.25)

p2 = ggplot(data=dt.melt[dt.melt$variable%like%"_SD" & dt.melt$SEQ_ID=="MMRF_2455_1_BM",], #MMRF_1325_1_BM
             aes(x=variable, y=value, fill=sig)) + 
  geom_bar(stat="identity") +
  ylim(-3,3) +
  xlab("Dimension") +
  ...
  theme(axis.text=element_blank(),
        ...
        axis.title=element_text(size=8))
p2
ggsave(filename = "plots/SBUK-17_MMRF-2455.png",plot = p2,width=2.5,height=1.25)
```
