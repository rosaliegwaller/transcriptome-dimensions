...
cnt = data.table(merge(tmp1,tmp2,by=c("decile"),all = T) %>% select("decile","n_no","n_yes"))
...
cnt = data.table(merge(merge(tmp1,tmp2,by=c("decile"),all = T),tmp3,by="decile",all=T) %>% select("decile","n_1","n_2","n_3"))
...
kable(cnt,align = 'c',caption = "Overall Surival PSL decile counts at 1000 days") # remove censored before 1000 days
...
kable(cnt,align = 'c',caption = "TTF PSL decile counts at 1000 days") # remove censored before 1000 days
