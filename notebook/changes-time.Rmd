...
melt.iss$id = as.integer(gsub("MMRF_","",melt.iss$PUBLIC_ID))
melt.iss$id = as.factor(melt.iss$id)

rng = c(min(mod.risk$iss$lp),max(mod.risk$iss$lp))

pI = ggplot(data=melt.iss,aes(x=Month,y=value,color=id)) +
...
melt.iss.first = melt.iss[Month==0,]
pI + geom_text(data=melt.iss.first,x=-3,aes(y=value,label=id),size=3,check_overlap = TRUE)
ggsave(pI,filename = "plots/iss-time.pdf",width=7.5,height=4)
```
...
for (i in ids) {
  p = ggplot(data=melt.iss[PUBLIC_ID==i,],aes(x=Month,y=value,group=variable)) +
        geom_line(size=1,color="gray",linetype=1) +
        geom_point(size=4) +  
...
p1 = ggplot(data=melt.os,aes(x=Month,y=value,color=id)) +
        ...
        #scale_color_manual(values = c("#9E0142","#D53E4F","#F46D43","#FDAE61","#FEE08B","#FFFFBF","#E6F598","#ABDDA4","#66C2A5","#3288BD","#5E4FA2")) +
...
p2 = p1 + geom_segment(data = melt.os.last,size=0.3,
  ...
  #scale_fill_manual(values = c("white","#F46D43","#66C2A5","#3288BD","#5E4FA2"))
...
```{r}
# old plots
p1 + geom_segment(data = melt.os.last, size = 0.5,
                  aes(color=PUBLIC_ID, x = Month, xend = smt, y = value, yend = value),linetype = 3) +
  geom_point(data = melt.os.last, size = 6, 
             aes(shape = censos, x = smt, y = value)) +
...
for (i in ids) {
  p = ggplot(data=melt.os[PUBLIC_ID==i,],aes(x=Month,y=value,group=variable)) +
        ...
        theme(legend.position = "right") +
        geom_segment(data = melt.os.last[PUBLIC_ID==i,], size = 1,
                     aes(x = Month, xend = smt, y = value, yend = value),linetype = 2) + 
        geom_point(data = melt.os.last[PUBLIC_ID==i,], shape = 22, size = 5,
                   aes(fill = censos, x = smt, y = value)) +
