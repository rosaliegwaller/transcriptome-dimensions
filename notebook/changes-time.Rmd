...
p1 = ggplot(data=melt.os,aes(x=Month,y=value,color=PUBLIC_ID)) +
        geom_line(size=1,linetype=1) +
        geom_point(size=4) + 
        ylim(rng) + xlim(c(0,80)) +
...
#p1 + geom_point(shape = 22, aes(fill = censos, y = smt, x = value))
...
p1 + geom_segment(data = melt.os.last, size = 1,
                  aes(color=PUBLIC_ID, x = Month, xend = smt, y = value, yend = value),linetype = 2) +
  geom_point(data = melt.os.last, shape = 22, size = 5, 
