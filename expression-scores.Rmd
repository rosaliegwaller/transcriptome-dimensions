...
```
Overall hazards ratio from PSL score
```{r}
summary(lm(data = actual_preds, formula = actual ~ predicted))
```
...
Waterfall High/Low risk score
```{r}
actual_preds$bin = as.factor(if_else(actual_preds$actual<2.424285,"low","high"))
setorder(actual_preds,-predicted)
w <- ggplot(actual_preds,aes(y=predicted-2.424285,x=1:nrow(actual_preds),fill=bin)) + # predicted score - high risk cutoff from clustering
  geom_bar(stat="identity") +
  scale_fill_manual(values = c("blue","gray"), name = "UAMS-70 Risk Group", labels = c("High (n=151)", "Low (n=616)")) +
